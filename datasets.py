...
class WCDataset(Dataset):
    ...
    def __init__(self, wmaps, cmaps, cmaps_f, cmaps_b, cmarkers_f, cmarkers_b, tmaps, wmap_lengths, cmap_lengths, cmap_w_lengths):
        """
        :param wmaps: padded encoded word sequences
        :param cmaps: padded encoded char sequences grouped by word
        ...
        :param cmap_w_lengths: character sequence lengths grouped by word
        """
        self.wmaps = wmaps
        self.cmaps = cmaps
        ...
        self.cmap_w_lengths = cmap_w_lengths

        self.data_size = self.wmaps.size(0)

    def __getitem__(self, i):
        return self.wmaps[i], self.cmaps[i], self.cmaps_f[i], self.cmaps_b[i], self.cmarkers_f[i], self.cmarkers_b[i], self.tmaps[i], \
                self.wmap_lengths[i], self.cmap_lengths[i], self.cmap_w_lengths[i]
