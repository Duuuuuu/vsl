...
def run(e):
    ...
    label_stats = train_helper.tracker(
        ["loss", "logloss", "kl_div", "sup_loss","vb_loss"])
    unlabel_stats = train_helper.tracker(
        ["loss", "logloss", "kl_div"])
    
    vb_temp = e.config.vb_temp
    ...
    for it in range(e.config.n_iter):
        ...
        l_loss, l_logloss, l_kld, sup_loss, vb_loss,\
        ...
        label_stats.update(
            {"loss": l_loss, "logloss": l_logloss, "kl_div": l_kld,
             "sup_loss": sup_loss, 'vb_loss': vb_loss}, l_mask.sum())
