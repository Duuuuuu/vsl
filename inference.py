...
class ViterbiDecoder():
    ...
    def decode(self, scores, lengths):
        ...
        backpointers = torch.ones((batch_size, max(lengths), self.tagset_size), dtype=torch.long, device=self.device) * self.end_tag
        ...
        pointer = torch.ones((batch_size, 1),
                             dtype=torch.long, device=self.device) * self.end_tag  # the pointers at the ends are all <end> tags
        ...
        # Sanity check
        assert torch.equal(decoded[:, 0], torch.ones((batch_size), dtype=torch.long, device=self.device) * self.start_tag)

        # Remove the <starts> at the beginning, and append with <ends> (to compare to targets, if any)
        decoded = torch.cat([decoded[:, 1:], torch.ones((batch_size, 1), dtype=torch.long, device=self.device) * self.start_tag],
                            dim=1)
